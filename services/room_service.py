...
class RoomService:
    ...
    def get_agent_rooms(self, agent_id: int) -> List[ChatRoom]:
        ...
        for membership in memberships:
            ...
            agent = self._database.get_agent(membership.room_id)
            if agent:
                room = ChatRoom(
                    id=agent.id,
                    name=f"{agent.id}" if not agent.is_architect else "The Architect",
    ...
    def send_message(self, room_id: int, sender_name: str, content: str,
                     message_type: str = "text", reply_to_id: int = None, sender_id: int = None) -> ChatMessage:
        ...
        Args:
            ...
            sender_id: Optional agent ID of sender (None for system messages)
        """
        ...
        logger.info(f"Message in room {room_id} from '{sender_name}' (ID: {sender_id})")
        return message
